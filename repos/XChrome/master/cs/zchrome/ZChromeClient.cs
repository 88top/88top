...
namespace XChrome.cs.zchrome
{
    ...
    public class ZChromeClient : IDisposable
    {
        ...
        public async Task<string> BuildArgsAsync(FingerprintConfig finger, XChromeClient xchrome,int debugPort)
        {
            ...
            //首页
            // ============【修改点 2：开始】============
            // 原逻辑：本地首页地址不带任何参数（args += " --new-window http://localhost:" + cs.Config.WellComePagePort;），
            // WelComePage 服务端收到请求时完全不知道这是哪个环境，只能把标题写死成通用的"欢迎"，
            // 多开时每个标签页标题都一样，分不清是环境几。
            //
            // 现改为：在本地首页地址后面带上当前环境的 ID（?id=xxx），
            // 这样 WelComePage.cs 收到请求后就能从 query string 里解析出 id，
            // 动态生成"环境：{id}"作为标题（具体见 WelComePage.cs 修改点3）。
            args += " --new-window http://localhost:" + cs.Config.WellComePagePort + "/?id=" + xchrome.Id;
            // ============【修改点 2：结束】============
            //启动位置
        ...
        public async Task<(bool isSuccess,string errMsg)> LaunchChromeAsync(string chromePath, string userDataDir,XChromeClient xchrome, FingerprintConfig finger, string extraArguments = "")
        {
            ...
            _ =Task.Run(async () =>
            {
                ...
                // ============【修改点 1：开始】============
                // 原逻辑：首页 target attach 成功后，立刻对首页 session 做一次顶层 Page.navigate，
                // 直接把本地首页（标题写死"环境：{ID}"）整个替换成 www.web3tool.vip 的详情页地址。
                // 一旦该域名访问不通，页面会一直转圈，标签页标题也会跟着变成网页 title / 一直转圈中，
                // 多开时完全看不出是哪个环境。
                //
                // 现改为：不再做顶层跳转。标签页从打开到关闭全程都固定停留在本地首页，
                // 标题固定显示"环境：{ID}"（见 WelComePage.cs 修改点3），
                // 不管 web3tool.vip 这个地址通不通，标题都不会再发生变化。
                //
                // if (_HomePageSession != "")
                // {
                //     try
