...
| **1 bit-identical (default, recommended)** | `Optimized=1` | **Bit-identical to the official runtime** (measured on two real game captures, 320 images) | Every acceleration that is **provably output-preserving**: 63 rewritten kernel variants + all cross-kernel fusions, the exact image-kernel patches (310.9: P1/P3/P4/P7), and `SkipRepeatedRealCopy=1` (the repeated full-resolution `OutputReal` copy inside one group is a dead store). Equivalent to `OptimizedKernels=1, ImagePatches=1, SkipRepeatedRealCopy=1, HardwareBilinear=0, ChainBlock0=0, LaunchChains=0, PlainVariant=0, DisableFusions=0`. |
...
|---|---|---|---|
| `SkipRepeatedRealCopy` | `0/1` | **set by the tier** (`1` from tier ≥1) | `1` skips the full-resolution `OutputReal` copy the runtime emits **repeatedly** inside one group (byte-for-byte identical after the first frame, a dead store). About −566 µs per real frame at 4K 6X, about −164 µs at 1080p 6X, 0 at 2X. **It is bit-identical by construction**: replaying two real captures gives 60/60 and 20/20 bit-identical `OutputReal` and 180/180 and 140/140 bit-identical outputs, so it is on by default from tier 1 up. The very few games that redraw the real frame between two Evaluates of the same group would in theory see a one-frame lag; the backend guards against this, forwarding as usual and logging `real_copy_mismatch` on anything suspicious — if it really happens, write this line as `0` explicitly. See the subsection below. Supported on both models. |
...
| `SpoofArchToGame` | absent / `0/1` | **absent = automatic** | Governs **only the game's own architecture gate**. **This is a tri-state key**: **not written** (the factory INI has no such line) = automatic, armed only after the backend decides "this is a Turing host", logging one `turing_host_defaults` and `arch_spoof_installed{trigger:"backend",default:true}` — so RTX 20 needs no edit at all to the factory INI, while **on a non-Turing card the automatic setting installs nothing and there is not a single `arch_spoof_*` record**; `1` = armed as soon as the INI has been read (earlier than the backend; if the game asks for the architecture before the DLSS-G runtime is loaded, only this setting is in time); `0` = never armed, logging one `arch_spoof_disabled` on Turing. At `1`, `nvapi64.dll`'s `nvapi_QueryInterface` export is redirected so that `NvAPI_GPU_GetArchInfo` reports Ada to the outside, which lets a game that hangs its frame-generation switch on Ada allow it (`nvngx_*`/`nvapi*` still see the real architecture, and no byte of NVIDIA code is changed). **It only rewrites when the real answer is exactly Turing (`0x160`)**: on other architectures the redirection is installed but does nothing, logging one `arch_spoof_inert{real_arch}`. `arch_spoof_installed` carries two fields, `trigger` and `default`: `trigger=settings` (written `1`, and `nvapi64.dll` was already in the process when the INI was read) / `nvapi_load` (written `1`, `nvapi64.dll` loaded later and was caught by the loader's LoadLibrary hook) / `backend` (armed only once the backend was installed); `default=true` means the automatic setting made the decision, and only `default=false` with `trigger=backend` means "`1` was written but armed very late". **It has nothing to do with the NGX core's `0xbad0000b`**: that one is the core comparing the GPU architecture against the minimum architecture the runtime exports (see the `fg_gate_create_feature` row below), a comparison the core has already finished during Streamline initialization, too early for any NVAPI spoof. |
...
Both runtimes' host graphs emit a full-resolution `CopyResource(Backbuffer → DLSSG.OutputReal)` at the end of **every** Evaluate. `ext_real` has a single writer and no reader and `ext_color` is read-only, so within one group every copy after the first writes byte-for-byte identical content — all dead stores. `1` makes the backend swallow it when `MultiFrameIndex > 1`. The gain grows with the multiplier (−566 µs per real frame at 4K 6X, 0 at 2X) and is orthogonal to kernel replacement (it works at `Optimized=0` too). It is on by default from tier 1 up (off at tier 0); turning it on assumes "the game does not redraw the real frame between two Evaluates of the same group"; the backend guards against this, forwarding as usual and logging `real_copy_mismatch` on anything suspicious. Mechanism in `docs/ARCHITECTURE.md`.
